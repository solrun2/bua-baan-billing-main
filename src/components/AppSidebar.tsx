import { BarChart3, FileText, CreditCard, Settings, ChevronDown, Receipt, FileX, ShoppingCart, PiggyBank, Building, Calculator } from "lucide-react";
...
export function AppSidebar() {
  ...
  return <Sidebar className="border-r border-border/40">
      <SidebarHeader className="border-b border-border/40 p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">Ket Document</h2>
      ...
      <SidebarContent className="p-4">
        <SidebarGroup>
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton asChild className={`${isActive('/') ? 'bg-primary text-black font-bold' : 'hover:text-black'} transition-colors`}>
          ...
          <Collapsible open={documentsOpen} onOpenChange={setDocumentsOpen}>
            <SidebarGroupLabel asChild>
              <CollapsibleTrigger className="flex items-center justify-between w-full p-2 text-muted-foreground hover:text-foreground transition-colors">
                ...
                <ChevronDown className={`w-4 h-4 transition-transform ${documentsOpen ? 'rotate-180' : ''}`} />
              </CollapsibleTrigger>
            </SidebarGroupLabel>
            <CollapsibleContent>
              <SidebarGroupContent>
                <SidebarMenu>
                  {documentItems.map(item => <SidebarMenuItem key={item.title}>
                      <SidebarMenuButton asChild className={`${isActive(item.url) ? 'bg-primary text-black font-bold' : 'hover:text-black'} transition-colors ml-4`}>
                        <Link to={item.url} className="flex items-center gap-3 p-2 rounded-lg">
        ...
        <SidebarGroup>
          <Collapsible open={financeOpen} onOpenChange={setFinanceOpen}>
            ...
            <CollapsibleContent>
              <SidebarGroupContent>
                <SidebarMenu>
                  {financeItems.map(item => <SidebarMenuItem key={item.title}>
                      <SidebarMenuButton asChild className={`${isActive(item.url) ? 'bg-primary text-black font-bold' : 'hover:text-black'} transition-colors ml-4`}>
                        <Link to={item.url} className="flex items-center gap-3 p-2 rounded-lg">
                          <item.icon className="w-4 h-4" />
                          <span>{item.title}</span>
